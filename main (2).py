...
def dopasowanie_czasu_do_nagrania():
    ...
    wave_path = 'C:/Users/chyre/Desktop/Studia/V/TM/Projekt2/potrzebne pliki/znormalizowane_nagrania'
    os.chdir(wave_path)
...
def normalizacja():
    ...
    for i in range(0, len(lista_z_danymi)):
        ...
        wav.write((path + 'znormalizowane_nagrania/' + str(lista_z_danymi[i][0])), 16000, nagranie)

    for j in range(0, len(lista_hukow)):
        fs, huk = wav.read(path + 'huk_wav/' + str(lista_hukow[j]))
        huk = huk / max(huk)
        wav.write((path + 'znormalizowane_huki/' + str(lista_hukow[j])), 16000, huk)
